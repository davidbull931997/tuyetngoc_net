...
function checkInput(a){$("#login-btn").hasClass("btn-primary")&&"silver"==$("#login-btn").css("background-color")&&"white"==$("#login-btn").css("color")||(a?""==$("input#username").val()||""==$("input#password").val()?swal("Oops...","Vui l\u00f2ng nh\u1eadp t\u00e0i kho\u1ea3n v\u00e0 m\u1eadt kh\u1ea9u","warning"):($("#login-btn").removeClass("btn-primary").css({"background-color":"silver",color:"white"}),$("div#login-page").css("display","none"),$("div.loader-parent").removeAttr("style"),$.post({url:"admin/adminLogin",
data:{username:$("input#username").val(),password:$("input#password").val()},success:function(a){a.status?(vue.$data.customerList=cl,vue.$data.customerList.sort(function(a,b){return b.playtime-a.playtime}),$("div#manage-page").css("visibility","hidden").fadeIn(400,function(){$("input#username").val("");$("input#password").val("");$("#custom-search-input > div > input").height($("#custom-search-input > div > input").height()-4);$("#custom-search-input > div > span > button").height($("#add-new-btn").height()+
18);resizeFixedTableHead();$("div.loader-parent").fadeOut(400,function(){$("div#manage-page").css("visibility","");wakeHerokuAppTimer=setInterval(function(){$.post("/admin/wakeHerokuApp")},12E5)})})):($("div.loader-parent").css("display","none"),$("#login-btn").addClass("btn-primary").removeAttr("style"),$("div#login-page").css("display",""),2==a.code?swal("Oops...","T\u00e0i kho\u1ea3n ho\u1eb7c m\u1eadt kh\u1ea9u sai!","error"):1==a.code&&swal("Oops...","T\u00e0i kho\u1ea3n \u0111ang \u0111\u01b0\u1ee3c s\u1eed d\u1ee5ng !",
"error"),glob_submit=!1)},async:!1})):userExecuteMaliciousScript())}function resizeFixedTableHead(){$("#fixed-thead > tr:nth-child(1) > th").width($("#manage-page > div > div > table > thead:nth-child(2) > tr:nth-child(1) > th").width());for(i=0;7>i;i++)$("#fixed-thead > tr:nth-child(2) > th:nth-child("+i+")").width($("#manage-page > div > div > table > thead:nth-child(2) > tr:nth-child(2) > th:nth-child("+i+")").width())}
